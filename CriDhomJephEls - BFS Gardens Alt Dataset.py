...
for i in range(k):
    if k > len(neighbors):
        ...
        break
    # print('The ', i+1, ' nearest park is: ', neighbors[i]['NAMES'])

# G = nx.Graph()

# for index, row in df.iterrows():
#     G.add_node(row['NAMES'], pos=(row['LATITUDE'], row['LONGITUDE']))
# for index, row in df.iterrows():
#     for i in range(len(neighbors)):
#         if row['NAMES'] == neighbors[i]['NAMES']:
#             G.add_edge(test_row[0], row['NAMES'], weight=i+1)
